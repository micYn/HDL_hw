...
module thumb (read_instruction_n, instruction_address,
  ...
  wire[`WORD_SIZE * `REG_FILE_SIZE - 1:0] R_flat;

  // SIGNAL DECLARATIONS used to aid in the Verilog description
  wire EX_branch_taken;       // indicates a branch has been taken
  wire [`WORD_SIZE-1:0] EX_branch_target; // branch target address
  ...
  wire [`WORD_SIZE-1:0] IF_PC;           // saved PC value
  ...
  IF step1(read_instruction_n, IF_IR, IF_PC,instruction_address, reset_n, clk, EX_branch_taken, EX_branch_target,instruction);
  ID step2(ID_cond,ID_opcode, ID_imm_offset, ID_Rn, ID_Rd, ID_Rd_code, ID_Rm_Rs, ID_PC, reset_n,clk, IF_IR, IF_PC, EX_branch_taken, R_flat);
  EX step3(data,read_data_n,write_data_n,EX_opcode,ALU_out,EX_ALU_out,EX_branch_taken,EX_branch_target,data_address,EX_imm_offset,EX_Rd_code, reset_n,clk,ID_opcode, ID_Rd, ID_Rm_Rs, ID_Rn, ID_imm_offset, ID_PC,ID_cond,ID_Rd_code,R_flat);
  WB step4(R_flat, reset_n,clk,EX_opcode,EX_ALU_out,EX_Rd_code,data,EX_imm_offset);
endmodule
/////////////////////////////////////////////////////////////////
module IF(
    ...
    output wire [`WORD_SIZE-1:0] instruction_address,
...
module ID(
    ...
    input wire branch_taken,
    input wire[`WORD_SIZE * `REG_FILE_SIZE - 1:0] R_flat
);
  reg [`WORD_SIZE-1:0] R [`REG_FILE_SIZE-1:0];
  integer i;
  // Unpack R_flat into R whenever R_flat changes
  always @(*) begin
  ...
  always @(posedge clk) begin
    if(~reset_n) begin
      ID_cond<=0; ID_opcode<=0; ID_imm_offset<=0; ID_Rn<=0; ID_Rd<=0; ID_Rd_code<=0; ID_Rm_Rs<=0; ID_PC<=0;
    end else begin
...
function condition_passed;
  ...
  always @(posedge clk) begin
    if (~reset_n) begin
      ...
      write_data_n <= 1'b1;
      EX_opcode <= 0; ALU_out =0; EX_ALU_out<=0; branch_taken<=0; branch_target<=0; data_address<=0; EX_imm_offset<=0; EX_Rd_code<=0;
...
module WB(
  inout wire[`WORD_SIZE * `REG_FILE_SIZE - 1:0] R_flat,
  input wire reset_n,
  input wire clk,
  ...
  //     end
  // end
  // Pack R back into R_flat whenever R changes
  genvar j;
  generate
      for (j = 0; j < `REG_FILE_SIZE; j = j + 1) begin : R_pack
          assign R_flat[j * `WORD_SIZE +: `WORD_SIZE] = R[j];
      end
  endgenerate
// WB: Write Back Stage (final stage)
  integer found_wb_i,wb_i;
  always @(posedge clk) begin
    if (~reset_n)
